...
if __name__ == "__main__":
    ...
    for ind, device in enumerate(device_list):
        active_device_captions[device.device_caption] = ind
    ...
    for round in round_list[first_shown_round:-1]:
        ...
        # protocol = api.download_file(round.round_protocol_path).decode("utf-16")
        protocol = api.action_protocol_round(round.round_id)
        protocol_dict = protocolxml_to_dict(protocol.protocol_body)
        for device_caption, index in active_device_captions.items():
            if device_caption in protocol_dict['Prescription'][0].keys():

                # for the last tray
                tray = protocol_dict['Tray'][-1]
